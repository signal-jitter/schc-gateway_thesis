...
extended with support for ICMP packet fields
parsing with ICMP type lengths possible
'''
...
#        self.sepacketHexaContent = packet
        ...
        print("\n\t\tFrame content (hexa): %s" % hexlify(packet))
        ...
        #firstByte = unpack('!BBHHBBQQQQHHHHBBH', packet[:104])
        firstByte = unpack('!BBHHBBQQQQBBHHH', packet)
...
########CHOISE  - UDP or ICMP
    ...
    # now the data
        ...
        if firstByte[4] == 58:
            print("ICMP MODE")
            if direction == "up":
                self.header_fields["ICMPV6.TYPE", 1]      = [firstByte[10], 8, 'fixed']
                self.header_fields["ICMPV6.CODE", 1]      = [firstByte[11], 8, 'fixed']
                self.header_fields["ICMPV6.CKSUM", 1]     = [firstByte[12], 16, 'fixed']
                self.header_fields["ICMPV6.IDENT", 1]     = [firstByte[13], 16, 'fixed']
                self.header_fields["ICMPV6.SEQNO", 1]     = [firstByte[14], 16, 'fixed']
