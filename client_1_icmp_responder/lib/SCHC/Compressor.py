...
extended for IP buffer parsing in value-send mode
former buffer for IP lengths(unsigned long long - 8 bytes) too small
--> is queried in apply function and set as parameter
'''
...
class Compressor:
    ...
    def CA_valueSent( self, buf, TV, FV, length, nature, arg ):
        ...
        if type( FV ) is int:
            print(FV)
            if arg == "ip":
                print("ip compress unlocked")
                map = struct.pack("!Q", FV )
                buf.add_bytes(map)
                return
...
# CA must be cleaned of argument MSB(4) => MSB and arg = 4
                ...
                if FID == "IPv6.iidES" or FID == "IPv6.iidLA" or FID == "IPv6.prefixES" or FID == "IPv6.prefixLA":
                    self.CompressionActions[CA]( buf, TV, FV, fieldLength, fixvar, "ip" )
                else:
